...
def cnn_forward(name, sent_pos, lexical, num_filters):
  with tf.variable_scope(name):
    input = tf.expand_dims(sent_pos, axis=-1)  # 扩维， [?, max_len, word_dim+pos_dim+pos_dim, 1]
    input_dim = input.shape.as_list()[2]        # word_dim+pos_dim+pos_dim
...
class CNN(BaseModel):
    ...
    def __init__(self, word_embed, data, word_dim, pos_num, pos_dim,
                 num_relations, keep_prob, num_filters, learning_rate, is_train):
        # load data
        ...
        lexical = tf.reshape(lexical, [-1, 6*word_dim])     # [batch_size, 6*word_dim]
        self.labels = tf.one_hot(rid, num_relations)        # [batch_size, num_relations]

        sentence = tf.nn.embedding_lookup(word_embed, sentence)     # [?, max_len, word_dim]
        pos1 = tf.nn.embedding_lookup(pos1_embed, pos1)     # [?, max_len, pos_dim]
        pos2 = tf.nn.embedding_lookup(pos2_embed, pos2)

    # sentence features
        sent_pos = tf.concat([sentence, pos1, pos2], axis=2)  # [?, max_len, word_dim+pos_dim+pos_dim]
        if is_train:
    ...
    # get_prediction
        logits, loss_l2 = linear_layer('linear_cnn', feature, feature_size,
                                       num_relations, is_regularize=True)
        prediction = tf.nn.softmax(logits)
        prediction = tf.argmax(prediction, axis=1)
        accuracy = tf.equal(prediction, tf.argmax(self.labels, axis=1))
        accuracy = tf.reduce_mean(tf.cast(accuracy, tf.float32))
        loss_ce = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels, logits=logits))
        ...
        with tf.control_dependencies(update_ops):
            # for batch_norm,
            # refer: https://stackoverflow.com/questions/33949786/how-could-i-use-batch-normalization-in-tensorflow/38320613
...
def build_train_valid_model(word_embed, train_data, test_data):
    with tf.name_scope("Train"):
        with tf.variable_scope('CNNModel', reuse=None):
            m_train = CNN(word_embed, train_data, FLAGS.word_dim,
    ...
    with tf.name_scope('Valid'):
        with tf.variable_scope('CNNModel', reuse=True):
            m_valid = CNN(word_embed, test_data, FLAGS.word_dim,
                    FLAGS.pos_num, FLAGS.pos_dim, FLAGS.num_relations,

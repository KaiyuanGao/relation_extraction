...
def input_attention(entity1, entity2, sentence, pos1, pos2, is_train=True):
    def _get_a(entity_embed, sentence):
        ...
        # return a_j
        A_j = tf.matmul(sentence, tf.expand_dims(entity_embed, -1))
        A_j = tf.reshape(A_j, [-1, FLAGS.max_len])
        alpha_j = tf.nn.softmax(A_j)
        return alpha_j

    def _get_z(pos1, pos2, sentence):
        res = tf.concat([sentence, pos1, pos2], axis=2)
        return res

    z = _get_z(pos1, pos2, sentence)
    a_1 = _get_a(entity1, sentence)
    a_2 = _get_a(entity2, sentence)
    alpha = (a_1 + a_2) / 2
...
class Model(BaseModel):
    def __init__(self, word_embed, data, num_classes, num_filter, position_dim, is_train=True, regulizer=True):
        ...
        position_embed = tf.get_variable('position_embed', shape=[2 * FLAGS.embed_num, position_dim],
        ...
        with tf.control_dependencies(update_ops):
            self.train_op = optimizer.minimize(loss, global_step)
...
def build_train_valid_model(train_data, test_data, word_embed):
    with tf.name_scope("Train"):
        with tf.variable_scope("model", reuse=None):
            mtrain = Model(word_embed, train_data, FLAGS.num_classes, FLAGS.num_filter,
                           FLAGS.position_dim, is_train=True, regulizer=True)

    with tf.name_scope("test"):
        with tf.variable_scope("model", reuse=True):
            mtest = Model(word_embed, test_data, FLAGS.num_classes, FLAGS.num_filter,
                          FLAGS.position_dim, is_train=False, regulizer=True)
    return mtrain, mtest

...
tf.flags.DEFINE_integer("max_len", 98, "max length of sentences")
tf.flags.DEFINE_integer("num_classes", 19, "number of relations")
tf.flags.DEFINE_integer("word_dim", 50, "word embedding size")
tf.flags.DEFINE_integer("num_epochs", 150, "number of epochs")
tf.flags.DEFINE_integer("batch_size", 100, "batch size")
tf.flags.DEFINE_integer("embed_num", 123, "number of position feature")
tf.flags.DEFINE_integer("position_dim", 25, "position embedding size")
tf.flags.DEFINE_integer("num_filter", 500, "cnn number of output unit")
tf.flags.DEFINE_integer("label_dim", 500, "label_dim")
tf.flags.DEFINE_float("learning_rate", 0.03, "learning rate")
tf.flags.DEFINE_float("keep_prob", 0.7, "dropout keep probability")
tf.flags.DEFINE_float("l2_learning", 0.0001, "l2_learning")

tf.flags.DEFINE_boolean('test', False, 'set True to test')
tf.flags.DEFINE_boolean('train', False, 'set True to train')
...
def main(_):
    with tf.Graph().as_default():
        train_data, test_data, word_embed = utils.inputs()

        m_train, m_val = hacnn.build_train_valid_model(train_data, test_data, word_embed)

        m_train.set_saver('hacnn-%d-%d-%f-%f' % (FLAGS.num_epochs, FLAGS.word_dim, FLAGS.learning_rate, FLAGS.keep_prob))
